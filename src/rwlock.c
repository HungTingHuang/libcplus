...
static void * rwlock_initialize_object(CPLUS_RWLOCK_CONFIG config)
{
    ...
    if (NULL != config->name && CPLUS_RWLOCK_MODE_NONE != config->mode)
    {
        ...
        case CPLUS_RWLOCK_MODE_CREATE:
            lock->shared_mem = cplus_sharedmem_create(rwlock_name, sizeof(struct named_rwlock));
            break;
...
{
    CPLUS_RWLOCK_CONFIG_T config = {0};
...
void unittest_rwlock(void)
{
    ...
    UNITTEST_ADD_TESTCASE(cplus_rwlock_xp_new, cross_process_test);
    ...
    UNITTEST_ADD_TESTCASE(cplus_rwlock_xp_new, bad_parameter);
