...
{
	struct mempool * mp = (struct mempool *)(obj);
	void * addr = CPLUS_NULL;
	uint32_t * p = CPLUS_NULL;
	CHECK_OBJECT_TYPE(obj);
...
{
	CHECK_IF(0 == block_count, CPLUS_NULL);
	CHECK_IF(sizeof(uint32_t) > block_size, CPLUS_NULL);
...
CPLUS_UNIT_TEST(cplus_mempool_alloc, functionity)
{
	...
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (mp = (cplus_mempool)cplus_mempool_new(5, sizeof(int)))));
	UNITTEST_EXPECT_EQ(5, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test0 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(4, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test1 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(3, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test2 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(2, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test3 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(1, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test4 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(0, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (test5 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (test6 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (test7 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (test8 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (test9 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
...
{
	cplus_mempool mp = CPLUS_NULL;
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (mp = (cplus_mempool)cplus_mempool_new(5, sizeof(struct test_block)))));
	UNITTEST_EXPECT_EQ(5, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block0 = (struct test_block *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(4, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block1 = (struct test_block *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(3, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block2 = (struct test_block *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(2, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->a, 0x00, sizeof(test_block0->a))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->b, 0x00, sizeof(test_block0->b))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->c, 0x00, sizeof(test_block0->c))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->d, 0x00, sizeof(test_block0->d))));
	...
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_cpy(&test_block1->a, b1s, sizeof(test_block1->a))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_cpy(&test_block1->b, b2s, sizeof(test_block1->b))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_cpy(&test_block1->c, b4s, sizeof(test_block1->c))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_cpy(&test_block1->d, b8s, sizeof(test_block1->d))));
	...
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block2->a, 0xff, sizeof(test_block2->a))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block2->b, 0xff, sizeof(test_block2->b))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block2->c, 0xff, sizeof(test_block2->c))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block2->d, 0xff, sizeof(test_block2->d))));
...
{
	cplus_mempool mp = CPLUS_NULL;
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(0, 4))));
	UNITTEST_EXPECT_EQ(EINVAL, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(5, 0))));
	UNITTEST_EXPECT_EQ(EINVAL, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(5, 1))));
	UNITTEST_EXPECT_EQ(EINVAL, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(5, 2))));
	UNITTEST_EXPECT_EQ(EINVAL, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(5, 3))));
	UNITTEST_EXPECT_EQ(EINVAL, errno);
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL == (mp = (cplus_mempool)cplus_mempool_new(3, GB))));
	UNITTEST_EXPECT_EQ(ENOMEM, errno);
}

CPLUS_UNIT_TEST(cplus_mempool_alloc, no_mempool)
{
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block0 = (struct test_block *)cplus_malloc(sizeof(struct test_block)))));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block1 = (struct test_block *)cplus_malloc(sizeof(struct test_block)))));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test_block2 = (struct test_block *)cplus_malloc(sizeof(struct test_block)))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->a, 0x00, sizeof(test_block0->a))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->b, 0x00, sizeof(test_block0->b))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->c, 0x00, sizeof(test_block0->c))));
	UNITTEST_EXPECT_EQ(true , (CPLUS_NULL != cplus_mem_set(&test_block0->d, 0x00, sizeof(test_block0->d))));
...
{
	cplus_mempool mp = CPLUS_NULL;
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (mp = (cplus_mempool)cplus_mempool_new_s(5, sizeof(int)))));
	UNITTEST_EXPECT_EQ(5, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test0 = (int32_t *)cplus_mempool_alloc(mp))));
	UNITTEST_EXPECT_EQ(4, cplus_mempool_get_free_blocks_count(mp));
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (test1 = (int32_t *)cplus_mempool_alloc(mp))));
...
CPLUS_UNIT_TEST(cplus_mempool_get_index, functionity)
{
	...
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (mp = (cplus_mempool)cplus_mempool_new_s(5, sizeof(uint32_t)))));
	for (int32_t idx = 0; idx < 5; idx++)
	{
		UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (addr[idx] = (int32_t *)cplus_mempool_alloc(mp))));
	}
	...
	UNITTEST_EXPECT_EQ(true, (CPLUS_NULL != (new_addr = (int32_t *)cplus_mempool_alloc(mp))));
