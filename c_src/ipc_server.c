...
static int32_t packet_analyze_completed(struct ipc_conn * ipc_conn)
{
	...
	struct ipc_conn_packet * completed_packet = &(ipc_conn->packet), response_packet;

	CPLUS_MEM_CLEAN(&(response_packet));
...
int32_t cplus_ipc_client_send_heartbeat(cplus_ipc_client obj, uint32_t timeout)
{
	...
	struct ipc_conn_packet request_packet, response_packet;
	CHECK_OBJECT_TYPE_EX(obj, OBJ_TYPE_CLIENT);

	CPLUS_MEM_CLEAN(&(request_packet));
	CPLUS_MEM_CLEAN(&(response_packet));
...
{
	struct ipc_client * clt = (struct ipc_client *)(obj);
	struct ipc_conn_packet resquest_packet;
	CHECK_OBJECT_TYPE_EX(obj, OBJ_TYPE_CLIENT);

	CPLUS_MEM_CLEAN(&(resquest_packet));
