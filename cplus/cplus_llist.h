...
#define CPLUS_LLIST_FOREACH(LLIST, ITERATOR) \
    for (uint32_t INDEX = 0 \
        ; NULL != LLIST && INDEX < cplus_llist_get_size(LLIST) \
        && (NULL != (ITERATOR = (typeof(ITERATOR))cplus_llist_get_of(LLIST, INDEX))) \
        ; INDEX++)
...
cplus_data cplus_llist_add_data_double(cplus_llist obj, char * key, double value);
